...
// laid out as three cards side by side (stacking on narrow screens), per
// the "scroll, not tabs or a step wizard" call. Cards map 1:1 onto the
// backend's three param groups (analysis/noise_impact.py ->
// TLModelParams / NoiseLevelParams / ExposureAssessmentParams): Scenario
// picks which precomputed site to use (site is a dropdown, not a map
// click -- see useNoiseImpact's module docstring), Pile driving is the
// noise-source params, Species is which hearing groups/impact
// types/metrics/depth range to evaluate thresholds for.
      ...
      >
        ...
        <div className="overflow-y-auto px-7 py-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Scenario -- which precomputed transmission-loss site */}
            <div className="h-80 shrink-0 border border-slate-200 dark:border-slate-700 rounded-lg p-4 flex flex-col gap-3 overflow-y-auto">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Scenario</h3>
              <label className="flex flex-col gap-1">
            ...
            {/* Pile driving -- the noise source itself. Fixed at 1 pile /
                24-hour assessment period for now (matches backend defaults,
                see analysis/noise_impact.py's compute_impact) rather than
                exposing those as adjustable fields. */}
            <div className="h-80 shrink-0 border border-slate-200 dark:border-slate-700 rounded-lg p-4 flex flex-col gap-2.5 overflow-y-auto">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Pile driving</h3>
            ...
            {/* Species -- which hearing groups/impact types/metrics to evaluate */}
            <div className="h-80 shrink-0 border border-slate-200 dark:border-slate-700 rounded-lg p-4 flex flex-col gap-1 overflow-y-auto">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1">Species</h3>
        ...
        {error && (
          <div className="mx-7 mb-4 px-3 py-2 rounded-md bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 text-xs text-red-600 dark:text-red-400 shrink-0">
            {error}
          </div>
        )}

        <div className="flex items-center justify-end gap-3 px-7 py-5 border-t border-slate-100 dark:border-slate-800 shrink-0">
          {!canRun && !running && (
