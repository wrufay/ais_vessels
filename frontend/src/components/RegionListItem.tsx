  ...
  return (
    <div
      className={`flex items-center gap-3 px-3 py-2.5 rounded-sm cursor-pointer transition ${
        highlighted ? "bg-slate-100 dark:bg-slate-800" : "hover:bg-slate-50 dark:hover:bg-slate-800"
      }`}
      onClick={onClick}
    >
      <input
      ...
      <div className={`min-w-0 ${onRemove ? "flex-1" : ""}`}>
        <div className={`text-sm font-inter text-slate-600 dark:text-slate-300 ${onRemove ? "truncate" : ""}`}>
          {label}
        </div>
        <div className="flex items-center gap-1.5 mt-0.5">
          <span className="w-2 h-2 rounded-full inline-block shrink-0" style={{ background: dotColor }} />
          <span className="text-[11px] text-slate-400 dark:text-slate-500 font-geologica">{tagLabel}</span>
        </div>
      </div>
      {onRemove && (
